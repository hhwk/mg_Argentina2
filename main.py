...
'Стартовая страница', 'Улучшения', 'Запуск ракет', 'Посещения', 'Гуманитарная помощь', 'Авторы'))
if pp['Atention'] != '':
...
if menu == 'Гуманитарная помощь':
    ...
    visit_money = st.selectbox('Кому вы хотите перевести деньги?',
                               ('Мексике', 'Канаде', 'Австралия', 'Швеции', 'Аргентине'))
    ...
    if city['reserch'] == '0':
        ...
        for l in range(0, len(country)):
            ...
            if country[l] == 'Аргентина':
                attak4 = st.multiselect('Какие города атакуем в Аргентина?',
                                        ['Буэнос-Сальта', 'Сальта', 'Кордова', 'Мендоса'])
        ...
        if st.button('Отправить данные'):
            if final_roket >= 0:
                ...
                Attak.put({'key': f'{ii}', 'Country': 'Мексика' + str(attak), 'Country1': 'Канада' + str(attak1),
                           'Country2': 'Австралия' + str(attak2), 'Country3': 'Швеция' + str(attak3),
                           'Country4': 'Аргентина' + str(attak4)})
...
if menu == 'Улучшения':
    ...
    sunks_for_who = st.multiselect('На какие страны вы хотите наложить санкции?',
                                   ['Мексика', 'Канада', 'Австралия', 'Швеция', 'Аргентина'])
    ...
    col1.metric('🏠' + city['shit1'] + masiv_shit[0] + f'{city_1}',
                '⚙️' + str(60 + 10 * city['up1'] + 10 * masiv_up[0]) + '%' + ' 🌳 ' + str(eco1) + '%', masiv_up[0] * 10)
    col2.metric('🏠' + city['shit2'] + masiv_shit[1] + f'{city_2}',
                '⚙️' + str(50 + 10 * city['up2'] + 10 * masiv_up[1]) + '%' + ' 🌳 ' + str(eco2) + '%', masiv_up[1] * 10)
    col3.metric('🏠' + city['shit3'] + masiv_shit[2] + f'{city_3}',
                '⚙️' + str(50 + 10 * city['up3'] + 10 * masiv_up[2]) + '%' + ' 🌳 ' + str(eco3) + '%', masiv_up[2] * 10)
    col4.metric('🏠' + city['shit4'] + masiv_shit[3] + f'{city_4}',
                '⚙️' + str(40 + 10 * city['up4'] + 10 * masiv_up[3]) + '%' + ' 🌳 ' + str(eco4) + '%', masiv_up[3] * 10)
    ...
    if st.button('Отправить данные'):
        if money >= 0:
            ...
            db.put({'key': f'{ii}', "money": money, "roket": number, "shit": str(masiv_shit), "up": str(masiv_up),
                    'sunks_for_who': str(sunks_for_who), 'reserch': reserch1})
            with st.spinner('Wait for it...'):
                time.sleep(1)
                st.success('Данные обновлены!')
